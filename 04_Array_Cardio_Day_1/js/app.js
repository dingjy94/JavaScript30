...
const sortByBirthDate = (a, b) => a.year - b.year;
inventors.sort(sortByBirthDate);
console.log('Sort the inventors by birthdate, oldest to youngest');
console.table(inventors);

// Array.prototype.reduce()
// 4. How many years did all the inventors live?
const allYearsLive = inventors.reduce(
  (acc, cur) => acc += (cur.passed - cur.year),
  0
);
console.log('How many years did all the inventors live?');
console.log(allYearsLive);

// 5. Sort the inventors by years lived
const sortByYearsLived = (a, b) => (a.passed - a.year) - (b.passed - b.year);
inventors.sort(sortByYearsLived);
...
// Array.from(document.querySelector('.mw-category').querySelectorAll('a'))
//   .map(link => link.textContent)
//   .filter((title) => title.includes('de'))

// 7. sort Exercise
// Sort the people alphabetically by last name
const sortByAlphabet = (a, b) => a.split(', ')[1] < b.split(', ')[1] ? -1 : 1;
names.sort(sortByAlphabet);
console.log('Sort the people alphabetically by last name');
console.log(names);
// 8. Reduce Exercise
// Sum up the instances of each of these
const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck'];
const sumUp = data.reduce((acc, cur) => {
  if (!acc[cur]) {
    acc[cur] = 0;
  }
